...
public class PanelGame  extends JPanel implements KeyListener{
	...
	public void paintComponent(Graphics g)
	{
		...
		if(panelC.getState()== panelC.getState().gamePanel  ) {
			for (int i=0; i < pics.getCurrentMapDimensions();i++)
			{
				...
				{
					if(pics.getCurrentMap()[i][j]== 'X') g.drawImage(pics.getWall(), j *100,i*100,100,100,this);
					...
					else g.drawImage(pics.getGround(), j*100, i*100, 100 , 100 ,this);}}paintCharacters(g);}}
	...
	public void paintOgre(Graphics g)
	{
		if(pics.getGame().getPlayingLevel().getNumberLevel()==2)
		{
			...
			{
				if(pics.getGame().getPlayingLevel().getOgres().get(i).isStun())g.drawImage(pics.getOgreStun(),pics.getGame().getPlayingLevel().getOgres().get(i).getYcoordinate() *100,pics.getGame().getPlayingLevel().getOgres().get(i).getXcoordinate() *100,100,100,this);
				
				else g.drawImage(pics.getOgre(),pics.getGame().getPlayingLevel().getOgres().get(i).getYcoordinate() *100,pics.getGame().getPlayingLevel().getOgres().get(i).getXcoordinate() *100,100,100,this);
				}} }
	...
	{
		if(arg0.getKeyCode() == KeyEvent.VK_UP) runButton('w');
		
		else if(arg0.getKeyCode() == KeyEvent.VK_RIGHT) runButton('d');
		
		else if(arg0.getKeyCode() == KeyEvent.VK_DOWN) runButton('s');
		
		else if(arg0.getKeyCode() == KeyEvent.VK_LEFT)runButton('a');
			
		else if(arg0.getKeyCode() == KeyEvent.VK_ESCAPE) System.exit(0);
